...
if __name__ == "__main__":
    ...
    mask = np.array(nibabel.load(f'/home/medicaldata/LJData/SR_ana/analysis_res/rois/parkROI/HCl.nii').get_fdata(),
                    dtype=bool)
    broadcast = model(
    ...
    # 将p值写入到每个体素上
    modelmask = image.load_img(f'/home/medicaldata/LJData/SR_ana/analysis_res/rois/parkROI/HCl.nii')
    par = corData[:, 9]
    niftiNew = makeNifti(modelmask, par)
    # niftiNew.to_filename(f'/home/medicaldata3/LJData/rbCue.rsa.derection/rbCue.7fold.brain.nii')
    print('newNifit saved done')

    ## 保存 p <0.05 的值
    corDataPval = np.where(corData[:, 9] > 0.05, 0, corData[:, 9])
    niftiNew = makeNifti(modelmask, corDataPval)
    # niftiNew.to_filename(f'/home/medicaldata3/LJData/rbCue.rsa.derection/rbCue.7fold.brainP.nii')

    ## 保存数据
    # pd.DataFrame(data=corData,
    #              columns=["x", "y", "z", "estimate", "2.5_CI", "97.5_CI", "std.error", "DF", "t-value", "p-value",
    #                       "Sig"]).to_csv(f"/home/medicaldata3/LJData/rbCue.rsa.derection/rbCue.7fold.brain.csv",
    #                                      sep=',',
