...
def sl_func(dat, msk, myrad, bcast_var):
    ...
    for data in dat:
        ...
        # 判断sl_data数据是否为空
        if np.any(np.isnan(rdm.dissimilarities)):
            r.append(0)
        else:
                                                                        # tau-a
...
if __name__ == "__main__":
    ...
    mask = np.array(nibabel.load(f'/home/medicaldata3/LJData/rsa/dire.ttest/MNI_mask.nii').get_fdata(),
                    dtype=bool)
    broadcast = model(np.loadtxt(open(f'/home/medicaldata/LJData/SR_ana/analysis_res/DerModel/Uptri_hudu_wCueModel-6fold.csv', "rb"),
                   delimiter=","))
    models = [broadcast]
    ...
    for subj in subj_list:
        data = [nibabel.load(
            f'/home/medicaldata3/LJData/rsa/wCue.rsa_direction/stats.{subj}_REML.nii').get_fdata().squeeze()[...,
                1:39:3]]
        ...
        # niftiNew.to_filename(f'/home/medicaldata3/LJData/rsa/wCue.rsa_direction/hudu/eucli/corr.fold6.{subj}.nii')
