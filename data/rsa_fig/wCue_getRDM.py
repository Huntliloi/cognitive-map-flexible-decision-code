...
for subj_name in subj_list:
    for run in ["wCue"]:
        ...
        data = data[..., 1:46:3].squeeze()  # x,y,z,15
        ...
        roi_masks = [
            np.array(load(f'/home/medicaldata/LJData/SR_ana/analysis_res/rois/SR4_wCue_diag_ROI/{roi}.nii').get_fdata(), dtype=bool)
            for roi in roi_list]
        # 循环控制在4个ROI上进行RSA
        for i in range(len(roi_list)):
            ...
            rdm = rsatoolbox.rdm.calc_rdm(dataset=rsadata, method="mahalanobis", descriptor="cond")
