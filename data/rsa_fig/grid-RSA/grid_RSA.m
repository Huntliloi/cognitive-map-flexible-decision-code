...
model.m4 = csvread(['E:\3、论文撰写\【表征学习】\数据及代码整理\Fig45_RSA\grid-RSA\model\RDM_du_SR3Model-4fold.csv']);
model.m5 = csvread(['E:\3、论文撰写\【表征学习】\数据及代码整理\Fig45_RSA\grid-RSA\model\RDM_du_SR3Model-5fold.csv']);
model.m6 = csvread(['E:\3、论文撰写\【表征学习】\数据及代码整理\Fig45_RSA\grid-RSA\model\RDM_du_SR3Model-6fold.csv']);
model.m7 = csvread(['E:\3、论文撰写\【表征学习】\数据及代码整理\Fig45_RSA\grid-RSA\model\RDM_du_SR3Model-7fold.csv']);
...
for i=1:length(current_model)
    for subj = 1:size(neuro_du,1)
           dsms = [cosmo_squareform(squeeze(neuro_du(subj,:,:)))', cosmo_squareform(eval(['model.',current_model{i}]))'];
           cc = cosmo_corr(dsms);
...
roi_results = [post.m4,post.m5,post.m6,post.m7];
%% Plot
figure('Color', 'white'); % 设置图形窗口背景为白色
[h, p, ci, stats] = ttest(roi_results, 0, 'Tail', 'both');
[h,p0(:,1), ci, stats(:,1)] = ttest2(roi_results(:,3), roi_results(:,1),'Tail', 'both');
[h,p0(:,2), ci, stats(:,2)] = ttest2(roi_results(:,3), roi_results(:,2),'Tail', 'both');
[h,p0(:,3), ci, stats(:,3)] = ttest2(roi_results(:,3), roi_results(:,4),'Tail', 'both');
%   P值统计校正
corrected_p = round(mafdr([p(3),p0], 'BHFDR', true),4);
P_model = corrected_p(1);
p1 = [corrected_p(2)];
p2 = [0,corrected_p(3),corrected_p(4)];
means = mean(roi_results);
errors = std(roi_results) ./ sqrt(size(roi_results, 1));
All = [(4:7)',means',errors',p'];
